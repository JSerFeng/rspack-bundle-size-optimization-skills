...
5. **Artifact check (reference-kind, agent-driven)** — the capture also wrote `post-loader-sources.jsonl`. List candidates with `node references/show-post-loader.template.cjs --list`, then for each unproven export open its reference sites with `node references/show-post-loader.template.cjs <module> --symbol <export>` and **read them** to judge genuine vs artifact. See "Reference-kind / artifact verification" below. The script shows source; the agent decides — no hard-coded detector.
...
### Reference-kind / artifact verification (is it *really* used?) — the agent reads post-loader source
...
- [references/show-post-loader.template.cjs](references/show-post-loader.template.cjs)
  Reads post-loader source on demand so the agent can inspect each reference site and judge genuine-vs-artifact use case by case: `--list` surfaces artifact-marker candidates; `<module> --symbol <export>` shows the actual lines referencing an export. It only shows source — the verdict is the agent's. This is the reference-kind / artifact check (decorator-metadata, polyfill, passthrough, …) done by reading, not by a hard-coded pattern.
